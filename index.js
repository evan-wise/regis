...
// User these to track user data for this proof of concept. :)
...
const main = async () => {
  ...
  client.on('message', msg => {
    ...
    if (
      msg.channel.name !== 'bot-playground' && msg.channel.type !== 'dm'
    ) {
      return;
    }
    ...
    const tokens = msg.content.toLowerCase().split(/\s+/).filter(t => t !== `@${client.user.name}`);
